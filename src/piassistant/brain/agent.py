...
class Agent:
    ...
    def _system_prompt(self) -> str:
        ...
        return (
            ...
            f"AUTO-DO: When a task is added, evaluate if YOU can do it right now using your tools.\n"
            f"Examples: 'check the weather' → get weather, 'add milk to grocery list' → add it, "
            f"'check my orders' → get_orders, 'what events do I have' → get_calendar_events, "
            f"'set a 10 min timer' → timer_set. If you can do it, do it immediately, mark the task complete "
            f"with task_complete, and tell the user the result. If it requires physical action or is beyond "
            f"your tools (e.g. 'buy groceries', 'call mom', 'fix the fence'), just save it as a task.\n"
